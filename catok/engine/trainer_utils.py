...
from catok.utils.lr_scheduler import build_scheduler
...
def get_fid_stats(real_dir, rec_dir, fid_stats):
    ...
    if fid_stats is None and real_dir is None:
        raise ValueError("fid_stats is None and real_dir is None; cannot compute FID.")
    kwargs = dict(
        input1=rec_dir,
        input2=real_dir,
        cuda=True,
    ...
    if fid_stats is not None:
        kwargs["fid_statistics_file"] = fid_stats
    stats = torch_fidelity.calculate_metrics(**kwargs)
    return stats
